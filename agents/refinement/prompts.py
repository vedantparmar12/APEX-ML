"""Prompts for the Refinement Agent"""
...
class RefinementPrompts:
    ...
    def get_ablation_prompt(self, code: str, previous_ablations: List[str]) -> str:
        """Get prompt for ablation study"""
        
        if previous_ablations:
            prev_ablations_str = "\n".join([
                f"## Previous ablation study result {i+1}\n{ablation}\n"
...
- The code should consequently print out which part of the code contributes the most to the overall performance."""
    ...
    def get_ablation_summary_prompt(self, ablation_code: str, output: str) -> str:
        """Get prompt for summarizing ablation results"""
...
- Identify which components contribute most to model performance.
- Be concise and focus on actionable insights."""
    
    def get_extract_block_prompt(self, code: str, ablation_summary: str, previous_refinements: List[Dict]) -> str:
        """Get prompt for extracting code block and planning improvement"""
        
        prev_blocks_str = ""
        if previous_refinements:
            prev_blocks_str = "\n# Previously refined code blocks:\n"
            for i, ref in enumerate(previous_refinements):
                prev_blocks_str += f"{i+1}. {ref['code_block'][:100]}...\n"
...
- Try to select a different code block than previously refined ones.
...
Return: list[Refine_Plan]"""
    ...
    def get_implement_plan_prompt(self, full_code: str, code_block: str, plan: str) -> str:
        """Get prompt for implementing improvement plan"""
        
        return f"""# Introduction
- You are a Kaggle grandmaster attending a competition.
- In order to win this competition, you need to improve a code block for better performance.
- We will provide the full Python solution, the code block you are improving, and the improvement plan.
...
# Your task
- Implement the improvement plan by modifying the full Python solution.
- Replace the identified code block with your improved version.
- Ensure the improved code integrates seamlessly with the rest of the solution.
- The improvement should follow the plan closely.
...
- There should be no additional headings or text in your response.
- Do not modify other parts of the code unnecessarily.
- Print out or return a final performance metric in your answer in a clear format with the exact words: 'Final Validation Performance: {{final_validation_score}}'."""
    ...
    def get_plan_refinement_prompt(self, code_block: str, previous_attempts_summary: str) -> str:
        """Get prompt for refining improvement plan"""
...
- The suggested plan should be different from the previous plans you have tried and should aim for a higher score.
